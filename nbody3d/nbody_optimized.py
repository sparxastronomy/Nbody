...
def accleration(pos, mass, G, softening):
    ...
    for i in range(N):
        for j in range(N):
            if inv_r3[i, j] > 0:
                inv_r3[i, j] = np.power(inv_r3[i, j], -1.5)
...
def energy(pos, vel, mass, G):
    ...
    # Kinetic Energy:
    vel_squared = np.zeros_like(mass)
    for i in range(N):
        vel_squared[i] = np.sum(np.square(vel[i]))

    # this becomes an array of form '[[. , ,]]'
    KE = 0.5 * np.dot(vel_squared.T, mass)
    ...
    inv_r = np.sqrt(np.square(dx) + np.square(dy) + np.square(dz))
    for i in range(N):
        for j in range(N):
            if inv_r[i, j] > 0:
...
def main(N, tEnd, dt, softening=0.1, energyplot=True, plotRealTime=True):
    ...
    dt           : time step
    softening    : Softening length, default = 0.1
    ...
    for i in tqdm(range(Nt), 'Number of iterations : '):
        # (1/2) kick
        vel += acc * dt/2.0
